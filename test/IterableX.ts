...
describe('IterableX', () => {
  ...
  it('ap', () => {
    ...
    const fab = IterableX.from([double, triple])
  ...
  it('chain', () => {
    const fa = IterableX.from([1, 2, 3])
    const fb = R.iterable.chain(fa, a => IterableX.from([a, a + 1]))
    assert.deepStrictEqual(toArray(fb), [1, 2, 2, 3, 3, 4])
  })

  it('filterMap', () => {
    const fa = IterableX.from([1, 2, 3])
    const fb = R.iterable.filterMap(fa, O.fromPredicate(n => n > 1))
    assert.deepStrictEqual(toArray(fb), [2, 3])
  })

  it('compact', () => {
    const fa = IterableX.from([1, 2, 3].map(O.fromPredicate(n => n > 1)))
  ...
  it('partitionMap', () => {
    const fa = IterableX.from([1, 2, 3])
    const s = R.iterable.partitionMap(fa, E.fromPredicate(n => n > 1, identity))
    assert.deepStrictEqual(toArray(s.left), [1])
    assert.deepStrictEqual(toArray(s.right), [2, 3])
  })

  it('separate', () => {
    const fa = IterableX.from([1, 2, 3].map(E.fromPredicate(n => n > 1, identity)))
